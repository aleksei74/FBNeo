...
#else
static inline void GTELOG(UINT32, const char *, ...) {}
#endif

static inline INT32 count_leading_zeros_32(UINT32 data)
{
	INT32 count = 0;

	for (INT32 bit = 31; bit >= 0; bit--) {
		if (data & (1U << bit)) break;
		count++;
	}
...
static inline INT32 count_leading_ones_32(UINT32 data)
{
	return count_leading_zeros_32(~data);
}
